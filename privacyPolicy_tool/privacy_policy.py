...
def read_string(single_language, StrID_index, single_language_index, ws):
    ...
    if os.path.exists(os.getcwd()+'\\index.html'):
        ...
        if False == os.path.exists(abspath+'\\Project\\mb20G\\privacy_policy\\'+single_language.lower()):
            run_sys_cmd('mkdir '+abspath+'\\Project\\mb20G\\privacy_policy\\'+single_language.lower())

        new_lang_path= abspath+'\\Project\\mb20G\\privacy_policy\\'+single_language.lower()+'\\'+single_language.lower()+'.html'
        ...
        if lines:
            for line in lines:
                ...
                if matchObj:
                    ...
                    if '${str_policy_page_title}' == matchObj.group(2)+"}":
                        page_title= get_language_string['${str_policy_main_title}'].replace('<h1>','').replace('</h1><br/>','')
                        line= line.replace('${str_policy_page_title}', page_title)
